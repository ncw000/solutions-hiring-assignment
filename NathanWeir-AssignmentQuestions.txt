...
Algolia's search technology works by storing your searchable data in a database in the cloud. Algolia then provides you with tools to perform search queries against that data. The place in which we store your data is called an 'index'.

Think of the Algolia database as a paper book of English dictionary words: the dictionary has an 'index' section, where the reader can quickly find the page number for a given word. When you access an Algolia index, you are in a sense grabbing a copy of this dictionary index in that you are receiving a tool that helps you find the specific data you need. The act of looking up an individual item in the index is known as 'indexing'.
...
As for metrics to use in custom ranking: you should think about these as, "What data can I provide Algolia so it can know which results I'd like users to see first?" Good choices for custom ranking metrics and attributes are those that indicate popularity. So, number of likes, number of positive reviews, etc. This will tell Algolia which results should be sorted to the top so that your users see them first.

I hope that clears things up for you. Please don't hesitate to reach out again should you have any more questions or want additional clarification.
...
I've sent your note along to the team, and we'll be sure to consider this issue as we continue updating the index management tools.
...
<Note to assignment reviewers: Potentially suggest automating clearing/deletion of indexes via API calls? I'd appreciate another pair of eyes to tell me if that's productive, as it doesn't solve the "this requires additional work" issue>
 ...
 * Log in to the Algolia dashboard and upload your searchable data to an Algolia search index.
